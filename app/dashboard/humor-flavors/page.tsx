...
export default async function HumorFlavorsPage() {
  ...
  const { data, count } = await supabase.from('humor_flavors').select('*', { count: 'exact' }).order('id')
    ...
    <div style={{ animation: 'fadeIn 0.3s ease' }}>
      <div style={{ padding: '28px 28px 22px', borderBottom: '1px solid var(--border)', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end' }}>
        <div>
          ...
          <h1 style={{ fontFamily: 'var(--sans)', fontSize: '26px', fontWeight: '800' }}>Humor Flavors</h1>
        </div>
