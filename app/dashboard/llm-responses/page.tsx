...
export default async function LLMResponsesPage() {
  ...
  const { data, count } = await supabase.from('llm_model_responses').select('*', { count: 'exact' }).order('created_datetime_utc', { ascending: false }).limit(200)
    ...
    <div style={{ animation: 'fadeIn 0.3s ease' }}>
      <div style={{ padding: '28px 28px 22px', borderBottom: '1px solid var(--border)', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end' }}>
        <div>
          ...
          <h1 style={{ fontFamily: 'var(--sans)', fontSize: '26px', fontWeight: '800' }}>LLM Responses</h1>
        </div>
      ...
      <div style={{ overflowX: 'auto' }}>
        ...
        {(!data || data.length === 0) && <div style={{ padding: '48px', textAlign: 'center', fontFamily: 'var(--mono)', fontSize: '12px', color: 'var(--text-dimmer)' }}>No responses found.</div>}
      </div>
    </div>
  )
}